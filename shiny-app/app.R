...
ui <- fluidPage(
  ...
  # Application title
  titlePanel("SDK Revenue - 30 days"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("relayer_premium", "SDK Premium:", .15, min = .0001, max = 1),
...
server <- function(input, output) {
  ...
  # model params
  # for this version of the model, we are focusing on SDK revenue only
  dao_premium <- .0001
  ...
  get_plot_data <- reactive({
    ...
    beta_relayer_earnings <- lapply(t, relayer_beta, mean_daily_txs=tx_volume, sd_daily_txs=sd_daily_txs,n_beta=n_beta, 
                                    ...
                                    dao_premium_gwei_price=dao_premium,
  ...
  # Total relay revenue hist plot
  output$allPlotHist <- renderPlot({
    
    ggplot(get_plot_data(), aes(x=totl_infra_rev_usd)) +
      geom_histogram() +
      scale_x_continuous(labels=scales::dollar_format()) +
      labs(title = paste0("Total Tx Revenue - ", n_beta," days"), x = "total revenue usd", y = "number of simulations")
